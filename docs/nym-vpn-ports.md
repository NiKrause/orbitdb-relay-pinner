...
If you reach your relay **through [Nym VPN](https://nymtech.net/)**, traffic exits the mixnet at a Nym node that enforces an **exit policy**: only connections to certain **remote destination ports** are allowed. The default relay ports (**9090–9093**) are **not** on that allow list, so browsers and clients tunneled via Nym will fail to connect.
